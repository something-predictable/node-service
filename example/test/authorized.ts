...
describe('authorized', () => {
    ...
    it('should authorize', async () => {
        using _ = allowErrorLogs()
        const response = await request(withBearer({ sub: 'its me' }, { uri: `authorized` }))
    ...
    it('should check', async () => {
        using _ = allowErrorLogs()
        const response = await request(withBearer({ sub: '🤫' }, { uri: `authorized` }))

        assert.strictEqual(response.status, 400)
    })

    it('should check', async () => {
        const response = await request(withBearer({ sub: '🤫' }, { uri: `authorized?q=stuff` }))
